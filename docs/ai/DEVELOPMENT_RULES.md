...
- **每次更新都要发版并上传**：任何 Bug 修复、新功能、UI 改进、依赖升级或安全修复都视为一次正式版本更新，必须递增版本号、更新详细日志、重新打包并自动上传 GitHub Release；不得把修复留在未发布的桌面产物中。发布权限失败时必须记录阻塞，不得宣称已上传
...
- **日志受众必须分离且客户日志要详尽**：根目录 CHANGELOG.md、RELEASE_NOTES.md 与 GitHub Release 给客户看，详细写清用户能看到什么、如何使用、兼容迁移、失败提示、限制和修复前后行为，但不写内部类名、密钥、测试噪音；docs/ai/CHANGELOG.md、CHANGE_HISTORY.md 与 CURRENT_STATUS.md 给开发者/AI 看，记录文件、根因、机制、迁移、测试、风险、权限阻塞和未完成事项
