...
> 依据：真实源码（2026-08-19，v2.3.0）。区分"已实现 / 部分实现 / 计划实现"，禁止把计划写成已实现。

## 已实现

### 写作模式
- 普通模式：8 步手动流程（论文信息 → 选题解析 → 资料检索 → 证据库 → 大纲 → 分章节写作 → 事实核查 → 输出）
- 全自动模式：一键完成 23 阶段（检索 → 证据库 → 证据核验 → 结构设计 → 分章写作 → 引用/参考文献双向检查 → 事实核查 → 质量检测 → 自然化修改 → 再次核查 → 自动质量判定 → 导出全部），自动重试/自动保存/暂停/取消/**断点恢复**
...
- 自动质量门（quality_gate.py）：统一判定是否满足开始时的结构、字数、引用、事实和风险要求，失败时明确标记“自动判定未达标”
...
- 响应式窗口与高 DPI 支持
- 稳定用户数据目录与旧版无损迁移、GitHub Release 异步更新检查和真实错误反馈
...
- 自动质量门和事实核查跳过/失败不误报通过
- `%LOCALAPPDATA%\\PaperAssistant\\paper_project` 稳定目录、配置缺失字段合并、冲突备份和迁移报告
- GitHub Release 更新检查入口、Claude/Gemini 请求回归、导出目录回退修复和 Windows workflow
